...
const PlaceOrder = () => {
  ...
  const handleSelectAddress = (address, productId) => {
    if (productId) {
      ...
      // ปิด dropdown หลังจากเลือกที่อยู่
      setIsAddressDropdownOpen(false);
    } else {
      // กรณีเลือกที่อยู่เริ่มต้น
      setSelectedAddress(address);
      // อัพเดทที่อยู่เริ่มต้นสำหรับทุกสินค้าที่ยังไม่มีที่อยู่กำหนด
      const newItemAddresses = { ...itemAddresses };
      cartItems.forEach((item) => {
        if (!newItemAddresses[item.productId]) {
          newItemAddresses[item.productId] = address;
        }
      });
      setItemAddresses(newItemAddresses);
    }
  ...
  const handleEditAddress = (address) => {
    console.log("Start editing address:", address); // Debug log
  ...
  const handleAddressSubmit = async (e) => {
    e.preventDefault();
    console.log("Submitting with editingAddressId:", editingAddressId); // Debug log
                    ...
                    >
                      <div className="flex flex-col lg:flex-row gap-8">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="mb-4 bg-gray-50 p-3 rounded-lg">
                              <div className="flex items-center justify-between mb-2">
                                ...
                                <button
                                  onClick={() =>
                                    setIsAddressDropdownOpen(product._id)
                                  }
                              ...
                              {itemAddresses[product._id] ? (
                                <div className="text-sm text-gray-600">
                                  <p className="font-medium">
                                    {itemAddresses[product._id].name}
                                  </p>
                                  <p>
                                    {itemAddresses[product._id].addressLine1}
                                  </p>
                                  {itemAddresses[product._id].addressLine2 && (
                                    <p>
                                      {itemAddresses[product._id].addressLine2}
                                    </p>
                                  )}
                                  <p>
                                    จ.{itemAddresses[product._id].district}, อ.
                                    {itemAddresses[product._id].province}
                                  </p>
                                  <p>{itemAddresses[product._id].postalCode}</p>
                                  <p>
                                    โทร:{" "}
                                    {itemAddresses[product._id].phoneNumber}
                                  </p>
                                </div>
                              ) : (
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          {isAddressDropdownOpen === product._id && (
                            <div
